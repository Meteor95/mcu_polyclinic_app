...
$(document).ready(function() {
    ...
    onload_datatables();
    onload_datatables_riwayat_penyakit_keluarga();
    onloadfromnavigation(param_nomor_identitas, param_nama_peserta);
});
function onload_datatables(){
    table = $("#datatables_riwayat_penyakit_keluarga").DataTable({
        paging: false,
        ordering: false,
        searching: false,
        info: false,
...
function onload_datatables_riwayat_penyakit_keluarga(){
    $.get('/generate-csrf-token', function(response) {
        $("#datatables_daftar_riwayat_penyakit_keluarga").DataTable({
            ...
            ajax: {
                "url": baseurlapi + '/pendaftaran/daftarpasien_riwayatpenyakitkeluarga',
                "type": "GET",
                    ...
                    render: function(data, type, row, meta) {
                        if (type === 'display') {
                            return `<div class="d-flex justify-content-between gap-2">
                                <button onclick="detail_data_riwayat_penyakit_keluarga('${row.user_id}','${row.transaksi_id}')" class="btn btn-success w-100">
                                    <i class="fa fa-eye"></i> Lihat Data
                                </button>
                                <button onclick="fill_form_riwayat_penyakit_keluarga_tabel('${row.user_id}','${row.transaksi_id}','${row.nomor_identitas}','${row.nama_peserta}')" class="btn btn-primary w-100">
                                    <i class="fa fa-edit"></i> Ubah Data
                                </button>
                                <button onclick="hapus_data_riwayat_penyakit_keluarga('${row.transaksi_id}','${row.nomor_identitas}','${row.nama_peserta}','${row.user_id}')" class="btn btn-danger w-100">
...
function clear_form(){
    ...
    $('#datatables_riwayat_penyakit_keluarga input').each(function() {
        $(this).val('');
    });
    $('#datatables_riwayat_penyakit_keluarga select').val('0'); 
}
$("#bersihkan_data_riwayat_penyakit_keluarga").on('click', function() {
    clear_form();
});
$("#simpan_riwayat_penyakit_keluarga").on('click', function() {
    ...
    Swal.fire({
        html: '<div class="mt-3 text-center"><dotlottie-player src="https://lottie.host/53c357e2-68f2-4954-abff-939a52e6a61a/PB4F7KPq65.json" background="transparent" speed="1" style="width:150px;height:150px;margin:0 auto" direction="1" playMode="normal" loop autoplay></dotlottie-player><div><h4>Konfirmasi Simpan Formulir RPK MCU</h4><p class="text-muted mx-4 mb-0">Apakah anda yakin ingin menyimpan formulir informasi Riwayat Penyakit Keluarga Peserta MCU dengan atas nama : <strong>'+$("#nama_peserta_temp_1").text()+'</strong> ?. Jika terjadi kesalahan silahakn ubah sesuai kebutuhan',
        ...
        if (result.isConfirmed) {
            ...
            table.rows().every(function() {
                let row = this.data();
                let rowId = row[0];
                let status = $(`#status_riwayat_penyakit_keluarga_${rowId}`).val();
                let keterangan = $(`#keterangan_riwayat_penyakit_keluarga_${rowId}`).val();
                let rowData = {
            ...
            $.get('/generate-csrf-token', function(response) {
                let data = {
                    ...
                    informasi_riwayat_penyakit_keluarga: bulkData
                };
                $.ajax({
                    url: baseurlapi + '/pendaftaran/simpanriwayatpenyakitkeluarga',
                    type: 'POST',
...
function fill_form_riwayat_penyakit_keluarga(user_id, transaksi_id,detail = false){
    $.get('/generate-csrf-token', function(response) {
        $.ajax({
            url: baseurlapi + '/pendaftaran/riwayatpenyakitkeluarga',
            type: 'GET',
            headers: { 'Authorization': 'Bearer ' + localStorage.getItem('token_ajax') },
            data: {
                _token: response.csrf_token,
            ...
            success: function(response) {
                ...
                if (detail){
                    ...
                    response.data.forEach(function(item) {
                        let row = $('#datatables_riwayat_penyakit_keluarga tbody tr').filter(function() {
                            return $(this).find('td:eq(0)').text() == item.id_atribut_pk;
                        });
                        if (response.data.length > 0) {
                            row.find('#status_riwayat_penyakit_keluarga_' + item.id_atribut_pk).val(item.status).trigger('change');
                            row.find('#keterangan_riwayat_penyakit_keluarga_' + item.id_atribut_pk).val(item.keterangan ? item.keterangan : "");
...
function detail_data_riwayat_penyakit_keluarga(user_id, transaksi_id){
    fill_form_riwayat_penyakit_keluarga(user_id, transaksi_id,true);
}
function fill_form_riwayat_penyakit_keluarga_tabel(user_id, transaksi_id, nomor_identitas, nama_peserta){
    ...
    fill_form_riwayat_penyakit_keluarga(user_id, transaksi_id);
}
function hapus_data_riwayat_penyakit_keluarga(transaksi_id, nomor_identitas, nama_peserta, user_id){
    Swal.fire({
    ...
    if (result.isConfirmed) {
        $.get('/generate-csrf-token', function(response) {
            $.ajax({
                url: baseurlapi + '/pendaftaran/hapusriwayatpenyakitkeluarga',
                type: 'DELETE',
                headers: { 'Authorization': 'Bearer ' + localStorage.getItem('token_ajax') },
                data: {
                    _token: response.csrf_token,
