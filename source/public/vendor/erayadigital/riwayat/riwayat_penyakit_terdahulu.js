...
function onload_datatables(){
    table = $("#datatables_penyakit_terdahulu").DataTable({
        paging: false,
        ordering: false,
        searching: false,
        info: false,
...
function onload_datatables_imunisasi(){
    $.get('/generate-csrf-token', function(response) {
        $("#datatables_daftar_penyakit_terdahulu").DataTable({
            searching: false,
            bProcessing: true,
            serverSide: true,
            ...
            ajax: {
                "url": baseurlapi + '/pendaftaran/daftarpasien_riwayatpenyakitterdahulu',
                "type": "GET",
                ...
                },
                {
                    ...
                    render: function(data, type, row, meta) {
                        if (type === 'display') {
                            ...
                            Tanggal Transaksi Penyakit Terdahulu: ${row.tanggal_transaksi_penyakit_terdahulu}<br>
                            `;
                        }
                        return data;
                    }
...
function clear_form(){
    ...
    $('#datatables_penyakit_terdahulu input').each(function() {
        $(this).val('');
    });
    $('#datatables_penyakit_terdahulu select').val('0'); 
}
$("#bersihkan_data_imunisasi").on('click', function() {
    clear_form();
});
$("#simpan_penyakit_terdahulu").on('click', function() {
    ...
    Swal.fire({
        html: '<div class="mt-3 text-center"><dotlottie-player src="https://lottie.host/53c357e2-68f2-4954-abff-939a52e6a61a/PB4F7KPq65.json" background="transparent" speed="1" style="width:150px;height:150px;margin:0 auto" direction="1" playMode="normal" loop autoplay></dotlottie-player><div><h4>Konfirmasi Simpan Formulir Riwayat Penyakit Terdahulu MCU</h4><p class="text-muted mx-4 mb-0">Apakah anda yakin ingin menyimpan formulir informasi Riwayat Penyakit Terdahulu Peserta MCU dengan atas nama : <strong>'+$("#nama_peserta_temp_1").text()+'</strong> ?. Jika terjadi kesalahan silahakn ubah sesuai kebutuhan',
        ...
        if (result.isConfirmed) {
            ...
            table.rows().every(function() {
                let row = this.data();
                let rowId = row[0];
                let status = $(`#status_penyakit_terdahulu_${rowId}`).val();
                let keterangan = $(`#keterangan_penyakit_terdahulu_${rowId}`).val();
                let rowData = {
            ...
            $.get('/generate-csrf-token', function(response) {
                let data = {
                    ...
                    informasi_penyakit_terdahulu: bulkData
                };
                $.ajax({
                    url: baseurlapi + '/pendaftaran/simpanriwayatpenyakitterdahulu',
                    type: 'POST',
                    ...
                    success: function(response) {
                       if (response.success){
                            clear_form();
                            $('#datatables_daftar_penyakit_terdahulu').DataTable().ajax.reload();    
...
function fill_form_imunisasi(user_id, transaksi_id,detail = false){
    $.get('/generate-csrf-token', function(response) {
        $.ajax({
            url: baseurlapi + '/pendaftaran/riwayatpenyakitterdahulu',
            type: 'GET',
            headers: { 'Authorization': 'Bearer ' + localStorage.getItem('token_ajax') },
            data: {
                _token: response.csrf_token,
            ...
            success: function(response) {
                ...
                if (detail){
                    ...
                    response.data.forEach(function(item) {
                        let row = $('#datatables_penyakit_terdahulu tbody tr').filter(function() {
                            return $(this).find('td:eq(0)').text() == item.id_atribut_pt;
                        });
                        if (response.data.length > 0) {
                            row.find('#status_penyakit_terdahulu_' + item.id_atribut_pt).val(item.status).trigger('change');
                            row.find('#keterangan_penyakit_terdahulu_' + item.id_atribut_pt).val(item.keterangan ? item.keterangan : "");
...
function hapus_data_imunisasi(transaksi_id, nomor_identitas, nama_peserta, user_id){
    ...
    if (result.isConfirmed) {
        $.get('/generate-csrf-token', function(response) {
            $.ajax({
                url: baseurlapi + '/pendaftaran/hapusriwayatpenyakitterdahulu',
                type: 'DELETE',
                headers: { 'Authorization': 'Bearer ' + localStorage.getItem('token_ajax') },
                data: {
                    _token: response.csrf_token,
