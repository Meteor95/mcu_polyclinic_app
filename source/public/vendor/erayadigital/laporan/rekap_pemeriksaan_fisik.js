...
$("#btn_baca_data_rekap").on('click', function() {
    $("#datatables_pemeriksaan_fisik_perusahaan").DataTable().ajax.reload();
});
function loadDataPasien() {
    $.get('/generate-csrf-token', function(response) {
        if ($.fn.DataTable.isDataTable('#datatables_pemeriksaan_fisik_perusahaan')) {
            $("#datatables_pemeriksaan_fisik_perusahaan").DataTable().destroy();
            $("#datatables_pemeriksaan_fisik_perusahaan").empty();
        }
        $("#datatables_pemeriksaan_fisik_perusahaan").DataTable({
            searching: false,
            lengthChange: false,
            ordering: false,
            bFilter: false,
            ...
            ajax: {
                "url": baseurlapi + '/laporan/rekap/pemeriksaan_fisik',
                "type": "GET",
            ...
            rowGroup: {
                ...
                startRender: function (rows, group) {
                    ...
                    let rowHeader = $('<tr class="group-header" style="background-color: #fafafa; font-weight: bold;">')
                        ...
                        .append('<td style="width:30%">Kelompok</td>')
                        .append('<td style="width:30%">Atribut</td>')
                        .append('<td style="text-align:center;width:20%">Normal</td>')
                        .append('<td style="text-align:center;width:20%">AbNormal</td>')
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function(data, type, row, meta) {
                        if (type === 'display') {
                            let text = row.nama_atribut.replace(/_/g, ' ');
