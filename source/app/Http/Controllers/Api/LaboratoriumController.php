...
use App\Models\Transaksi\{UnggahanCitraLab, Transaksi as TransaksiMCU};
...
class LaboratoriumController extends Controller
{
    ...
    public function detail_tindakan(Request $req){
        try {
            ...
            $id_transaksi_lab = $req->id_transaksi;
            if ($req->nomor_mcu != null || $req->nomor_mcu != '') {
                $nomor_mcu = $req->nomor_mcu;
                $id_transaksi_mcu = TransaksiMCU::where('no_transaksi', $nomor_mcu)->first()->id;
                $id_transaksi_lab = Transaksi::where('no_mcu', $id_transaksi_mcu)->first()->id;
            }
            if ($validator->fails()) {
                $dynamicAttributes = ['errors' => $validator->errors()];
                return ResponseHelper::error_validation(__('auth.eds_required_data'), $dynamicAttributes);
            ...
            $transaksiData = Transaksi::join('transaksi_detail', 'transaksi.id', '=', 'transaksi_detail.id_transaksi')
                ...
                ->where('transaksi.id', $id_transaksi_lab)->get();
            $transaksiFee = Transaksi::where('id_transaksi', $id_transaksi_lab)->join('transaksi_fee', 'transaksi.id', '=', 'transaksi_fee.id_transaksi')->get();
            $berkasLaboratorium = UnggahanCitraLab::where('id_trx_lab', $id_transaksi_lab)->get();
