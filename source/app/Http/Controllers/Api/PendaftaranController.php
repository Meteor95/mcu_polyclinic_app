...
class PendaftaranController extends Controller
{
    ...
    public function getdatapeserta(Request $request)
    {
        try {
            ...
            $data_peserta = Peserta::where('nomor_identifikasi', $request->nomor_identitas)->first();
            $dynamicAttributes = [  
                'data_identitas' => $data,
                'data' => $data_peserta,
            ...
            if ($data) {
                ...
                $dynamicAttributes = [  
                    'data' => $data_peserta,
                    'message_info' => '<h4>Informasi Peserta dengan Nama : <strong>'.$data_peserta->nama_peserta.'</strong></h4><span style="color:red">BELUM TERDAFTAR PADA SISTEM MCU</span>. Informasi member ini akan ditambahkan menjadi member di '.config('app.name').' secara otomatis jika selesai melakukan transaksi MCU',
                ];
                if ($data_peserta) {
