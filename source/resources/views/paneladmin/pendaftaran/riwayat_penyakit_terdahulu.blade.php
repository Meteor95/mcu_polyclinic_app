    ...
    <div class="col-sm-12">
      <div class="card">
        <div class="card-header">
          @include('komponen.information_user', ['title_card' => "Penyakit Terdahulu", 'informasi_apa' => "informasi penyakit terdahulu"])
        </div>
        <div class="card-body">
          <div class="row formulir_group">
            ...
            <table class="table display" id="datatables_penyakit_terdahulu">
              ...
              <tbody>
                @foreach ($data['penyakit_terdahulu'] as $item)
                <tr>
                  <td>{{$item->id}}</td>
                  <td>{{$item->nama_atribut_pt}}</td>
                  <td>
                    <select id="status_penyakit_terdahulu_{{$item->id}}" class="form-select">
                      <option value="1">Ya</option>
                      <option value="0" selected>Tidak</option>
                    </select>
                  </td>
                  <td>
                    <input type="text" class="form-control" placeholder="Jika Ada" id="keterangan_penyakit_terdahulu_{{$item->id}}">
                  </td>
                </tr>
                @endforeach
              </tbody>
            ...
            <div class="d-flex justify-content-between gap-2 background_fixed_right_row">
              <button class="btn btn-danger w-100 mt-3 formulir_group_button" id="bersihkan_data_penyakit_terdahulu"><i class="fa fa-refresh"></i> Bersihkan Data</button>                   
              <button class="btn btn-success w-100 mt-3 formulir_group_button" id="simpan_penyakit_terdahulu"><i class="fa fa-save"></i> Simpan Data</button>                   
            </div>
...
<script src="{{ asset('vendor/erayadigital/riwayat/riwayat_penyakit_terdahulu.js') }}"></script>
<script>
